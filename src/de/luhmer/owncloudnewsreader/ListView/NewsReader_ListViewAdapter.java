...
public class NewsReader_ListViewAdapter extends BaseAdapter {
	List<NewsReader_Row> rows;		
	LayoutInflater inflator;
	//ToDoList toDoList; 
	
	public NewsReader_ListViewAdapter(Context context) {
		super();
		rows = new ArrayList<NewsReader_Row>();
	...
	*/
	...
	public void ReplaceRow(ConcreteSubscribtionItem vTodo, int position)
	{
		...
		rows.add(position, new NewsReader_CLI(vTodo, inflator));
		this.notifyDataSetChanged();
	}
		
	public ArrayList<NewsReader_Row> convertItems(ArrayList<ConcreteSubscribtionItem> items, Activity activity)
	{
		ArrayList<NewsReader_Row> temp = new ArrayList<NewsReader_Row>();
		
		for (ConcreteSubscribtionItem vtodo : items) {			
			temp.add(new NewsReader_CLI(vtodo, inflator));
		}
